    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('store.coupons.store') }}">
                        @csrf
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" name="description" rows="3"
                                ...
                                placeholder="クーポンの詳細説明">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="discount_type" name="discount_type" required
                                ...
                                <option value="percentage" {{ old('discount_type') === 'percentage' ? 'selected' : '' }}>パーセント割引</option>
                                <option value="amount" {{ old('discount_type') === 'amount' ? 'selected' : '' }}>金額割引</option>
                                <option value="free_item" {{ old('discount_type') === 'free_item' ? 'selected' : '' }}>無料商品</option>
                            </select>
                            @error('discount_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mt-4">
                            ...
                            <input id="discount_value" type="number" name="discount_value" value="{{ old('discount_value') }}" required min="0" step="0.01"
                        ...
                        <div class="mt-4">
                            ...
                            <input id="code" type="text" name="code" value="{{ old('code') }}"
                                class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-orange-500 focus:border-orange-500 @error('code') border-red-500 @enderror"
                                placeholder="空欄の場合は自動生成されます">
                            <p class="mt-1 text-xs text-gray-500">空欄にすると8文字のランダムなコードが自動生成されます</p>
                            @error('code')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input id="usage_limit" type="number" name="usage_limit" value="{{ old('usage_limit') }}" min="1"
                            ...
                            <p class="mt-1 text-xs text-gray-500">空欄にすると使用回数が無制限になります</p>
                            @error('usage_limit')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="conditions" name="conditions" rows="3"
                                ...
                                placeholder="例: 5,000円以上のご利用で使用可能">{{ old('conditions') }}</textarea>
                            @error('conditions')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
